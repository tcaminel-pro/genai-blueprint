...
class DemoConfigEditor(BaseModel):
        ...
        for idx, item in enumerate(items):
            ...
            with col1:
                ...
                for field_key, field_value in item.items():
                    edited_item[field_key] = DemoConfigEditor.render_field(
                        field_key, field_value, f"{key}_{idx}"
                    )
    ...
    def main():
        ...
        with raw_tab:
            ...
            st.code(yaml.dump(current_data, default_flow_style=False, sort_keys=False, allow_unicode=True), language="yaml")
        ...
        st.sidebar.info(
            f"**File Path:**\n`{selected_file}`\n\n"
            f"**File Size:** {selected_file.stat().st_size:,} bytes"
        )
