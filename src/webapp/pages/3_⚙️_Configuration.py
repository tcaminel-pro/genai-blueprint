...
def llm_configuration_section() -> None:
    ...
    with col1:
        debug_mode = st.checkbox(
            "Debug Mode",
            value=False,
            help="Enable LangChain debug mode for detailed logging"
        )
    ...
    with col2:
        verbose_mode = st.checkbox(
            "Verbose Mode",
            value=True,
            help="Enable LangChain verbose mode for operation details"
        )
...
def monitoring_configuration_section() -> None:
    ...
    if "LUNARY_APP_ID" in os.environ:
        lunary_enabled = st.checkbox(
            "Use Lunary.ai for monitoring",
            value=False,
            disabled=True,
            help="Lunary monitoring (currently disabled)"
        )
        if lunary_enabled:
    ...
    if "LANGCHAIN_API_KEY" in os.environ:
        langsmith_enabled = st.checkbox(
            ...
            value=True,
            help="Enable LangSmith tracing and monitoring"
        )
    ...
    if not monitoring_options:
        st.info("No monitoring services detected. Set LUNARY_APP_ID or LANGCHAIN_API_KEY environment variables to enable monitoring.")
