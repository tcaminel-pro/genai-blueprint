...
class ResearchAgentExample:
    ...
    def create_agent(self, custom_instructions: Optional[str] = None):
        ...
        instructions = custom_instructions or """You are an expert researcher specializing in comprehensive, accurate research.
...
"""
        ...
        if custom_instructions:
            ...
            self.agent = deep_agent_factory.create_agent(
                config=config,
                tools=[search_tool] + additional_tools,
                async_mode=True
            )
    ...
    async def research_with_context(
        ...
        additional_questions: Optional[list] = None
    ) -> Dict[str, Any]:
        """
...
async def main():
    ...
    result = await researcher.research_topic(
        topic="LangGraph framework for building AI agents",
        depth="quick"
    )
