...
class PydanticRag(PydanticRagBase):
    ...
    def create_vector_search_smolagent_tool(self) -> Tool:
        """Create a SmolAgents Tool for searching the vector store."""
        _entity_id_name: str = self._key_field.split(".")[-1]
        _top_class_description: dict = self.get_top_class_fields()
        _entity_key_name: str = self._key_field.split(".")[-1]

        class VectorSearchSmolAgentTool(Tool):
            """Tool for searching the vector store for semantic matches in SmolAgents."""
            ...
            def forward(self, query: str, selected_sections: List[str], entity_keys: List[str] = None) -> str:
                """Execute search against the vector store."""
        ...
        return VectorSearchSmolAgentTool()
