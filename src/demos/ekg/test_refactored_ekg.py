...
from rainbow_subgraph import get_subgraph, SUBGRAPH_REGISTRY
...
def main() -> None:
    ...
    details_table.add_row(
        "Node Types", 
        str(len(node_labels)), 
        ", ".join(list(node_labels.keys())[:3]) + "..."
    )
    details_table.add_row(
        "Relationships", 
        str(len(relationship_labels)), 
        ", ".join(list(relationship_labels.keys())[:3]) + "..."
    )
    details_table.add_row(
        "Sample Queries", 
        str(len(sample_queries)), 
        sample_queries[0][:50] + "..." if sample_queries else "None"
    )
